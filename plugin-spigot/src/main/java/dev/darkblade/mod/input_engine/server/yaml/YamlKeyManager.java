 ...
 *
 * <p>On startup, this class reads all {@code .yml} files from the
 * {@code plugins/InputEngine/keys/} directory, parses them into
 * {@link YamlKeybind} records, and registers them with the plugin's
 * network system so they are sent to connected clients.</p>
 *
 * <p>If the {@code keys/} directory is empty on first run, an example
 * configuration file is generated automatically.</p>
 */
public class YamlKeyManager {
    ...
    /**
     * Parses a list of maps from the YAML into a list of {@link ConfigurationSection} objects.
     *
     * <p>Each map in the YAML list (e.g. {@code conditions} or {@code actions}) is converted
     * into an in-memory {@link MemoryConfiguration} so that handlers can read values using
     * the familiar {@link ConfigurationSection} API.</p>
     */
