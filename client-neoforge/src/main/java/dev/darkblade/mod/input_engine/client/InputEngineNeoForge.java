            ...
            if (isCurrentlyPressed && !wasPressed) {
                ...
                if (state.requiresDoubleTap) {
                    if (currentTime - state.lastReleaseTime <= 300) {
                        ...
                        if (conn != null && conn.hasChannel(KeystrokePayload.TYPE)) conn.send(new net.minecraft.network.protocol.common.ServerboundCustomPayloadPacket(new KeystrokePayload(actionId, true, 0, true)));
                        state.lastReleaseTime = 0;
                    }
                } else {
                    var conn = net.minecraft.client.Minecraft.getInstance().getConnection();
                    if (conn != null && conn.hasChannel(KeystrokePayload.TYPE)) conn.send(new net.minecraft.network.protocol.common.ServerboundCustomPayloadPacket(new KeystrokePayload(actionId, true, 0, false)));
                }
                ...
                if (state.trackHoldDuration && holdDuration > 0) {
                    var conn = net.minecraft.client.Minecraft.getInstance().getConnection();
                    if (conn != null && conn.hasChannel(KeystrokePayload.TYPE)) conn.send(new net.minecraft.network.protocol.common.ServerboundCustomPayloadPacket(new KeystrokePayload(actionId, false, holdDuration, false)));
                } else if (!state.requiresDoubleTap) {
                    var conn = net.minecraft.client.Minecraft.getInstance().getConnection();
                    if (conn != null && conn.hasChannel(KeystrokePayload.TYPE)) conn.send(new net.minecraft.network.protocol.common.ServerboundCustomPayloadPacket(new KeystrokePayload(actionId, false, 0, false)));
                }
            }
        }
    }
